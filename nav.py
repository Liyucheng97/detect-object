...
class NavTest():
    def __init__(self):
        ...
        locations['point_01'] = Pose(Point(-2.000, 0.000, 0.000),
                                  Quaternion(0.000, 0.000, -0.309, 0.951))
        locations['hall_kitchen'] = Pose(Point(-4.000, -4.000, 0.000),
                                   Quaternion(0.000, 0.000, -0.816, 0.578))
        locations['hall_bedroom'] = Pose(Point(1.000, -5.000, 0.000),
                                   Quaternion(0.000, 0.000, -0.003, 1.000))
 
        locations['hall_foyer'] = Pose(Point(3.000, -1.700, 0.000),
	    ...
	    # check for whether the object is detected
	    rospy.Subscriber("chatter",String, self.detect_Callback)
	...
	if data.data == "True":
	    ...
	    rospy.loginfo("detect successfully,Stopping the robot...")
	    self.move_base.cancel_goal()
	    rospy.sleep(2)
	    self.cmd_vel_pub.publish(Twist())
	    rospy.sleep(1)
	    rospy.signal_shutdown("detect successfully")
def trunc(f, n):
